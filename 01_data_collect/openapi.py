...
def businfo():
    ...
    resp_cn = requests.get(url)
    resp_cn_json = resp_cn.json()
    count = int(resp_cn_json['tbisMasterStation']['list_total_count'])
    count_num = math.trunc(count/1000) + 1
    for _ in range(count_num):
        url = f'http://openapi.seoul.go.kr:8088/{serviceKey}/json/tbisMasterStation/{startpg}/{endpg}/'
        startpg = endpg + 1
...
def buscountinfo():
    ...
    for _ in range(count_num):
        ...
        bus.append(stat)
    total=dict()
    total["buscountinfo"] = bus
    with open('buscountinfo.json', 'w') as f:
        json.dump(total, f)
...
def bike_station():
    url = f'http://openapi.seoul.go.kr:8088/{serviceKey}/json/bikeStationMaster/{startpg}/{endpg}/'
    resp_cn = requests.get(url)
    resp_cn_json = resp_cn.json()
    count = int(resp_cn_json['bikeStationMaster']['list_total_count'])
    count_num = math.trunc(count/1000) + 1
    for _ in range(count_num):
        url = f'http://openapi.seoul.go.kr:8088/{serviceKey}/json/bikeStationMaster/{startpg}/{endpg}/'
        startpg = endpg + 1
        ...
        response = requests.get(url)
        rjson = response.json()
        stat = rjson['bikeStationMaster']
        with open('bike_station.json', 'a') as f:
            json.dump(stat, f)
